...
Pool.prototype.status = function() {
  ...
  _.each(self._pool, function(connobj) {
    var conn = connobj.conn;
    var output = "  UUID: " + connobj.uuid;
    conn.isClosed(function(err, result) {
      if (err) output += ", CLOSED: " + err;
      output += ", CLOSED: " + result
      conn.isValid(1000, function(err, result) {
        if (err) output += ", VALID: " + err;
        output += ", VALID: " + result
      });
      console.log(output);
    });

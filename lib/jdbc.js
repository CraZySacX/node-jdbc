...
JDBCConn.prototype.initialize = function(config, callback) {
  ...
  for(var name in self._config.properties) {
    properties.putSync(name, self._config.properties[name]);
  }

  self._props = properties;

  if (self._config.user && self._props.getPropertySync('user') === undefined) {
    self._props.putSync('user', self._config.user);
  }

  if (self._config.password && self._props.getPropertySync('password') === undefined) {
    self._props.putSync('password', self._config.password);
  }

  self._pool = new Pool(self._config);
  self._pool.initialize(function(err) {
    if (err) {
      return callback(err);
    } else {
...
JDBCConn.prototype.open = function(callback) {
  ...
  dm.getConnection(self._config.url, function (err, conn) {
    if (err) {
      return callback(err);
    } else {
      self._conn = conn;

...
JDBCConn.prototype.initialize = function(config, callback) {
  ...
  if (self._config.user){
    if(self._props.getPropertySync('user') === undefined){
      self._props.putSync('user', self._config.user);
    }
  }

  if (self._config.password){
    if(self._props.getPropertySync('password') === undefined){
...
JDBCConn.prototype.open = function(callback) {
  ...
  dm.getConnectionWithProps(self._config.url, self._config.props, function (err, conn) {
    if (err) {
      return callback(err);
    } else {
      self._conn = conn;

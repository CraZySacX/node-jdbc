...
function getConnection(self) {
  return java.callStaticMethodSync('java.sql.DriverManager', 'getConnection', self._config.url, self._props);
}

function createPool(self) {
  if (self._pool.available.length == 0) {
    console.log("Creating Connection Pool");
    if (self._config.minpoolsize) {
      for(i = 0; i < self._config.minpoolsize; i++) {
        self._pool.available.unshift(getConnection(self));
      }
    } else {
      self._pool.available.unshift(getConnection(self));
    }
  ...
  console.log("Created " + self._pool.available.length + " connection(s)");
}
...
JDBCConn.prototype.initialize = function(config, callback) {
  ...
  java.newInstance(self._config.drivername, function(err, driver) {
    if (err) {
      ...
      dm.registerDriver(driver, function(err, registered) {
        if (err) {
          return callback(err);
        } else {
          return callback(err, self._config.drivername);
        }
      });
    }
  });
...
JDBCConn.prototype.open = function(callback) {
  ...
  self._conn = getConnection(self);

  if (self._conn) {
    return callback(null, self._conn);
  } else {
...
JDBCConn.prototype.executeQuery = function(sql, callback) {
  ...
  self._conn.createStatement(function(err, statement) {
    if (err) {
      ...
      if (self._config.fetchsize) {
        statement.setFetchSizeSync(self._config.fetchsize);
      }
...
JDBCConn.prototype.callProcedure = function(proc, callback) {
  var self = this;

  self._conn.prepareCall(proc, function(err, callableStatement) {
    if (err) {
      return callback(err);
    } else {
      callableStatement.execute(function(err, result) {
        if (err) {
          ...
          if (result) {
            // TODO: process results here if result is true.
            return callback(null, callableStatement);
          } else {
            return callback(null, result);
          }
        }

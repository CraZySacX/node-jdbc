...
PreparedStatement.prototype.setObject = function(index, val, targetSqlType, scaleOrLength, callback) {
  if (targetSqlType) {
    if (scaleOrLength) {
      this._ps.setObject(index, val, targetSqlType, scaleOrLength, function(err) {
        if (err) return callback(err);
        callback(null);
      });
    } else {
      this._ps.setObject(index, val, targetSqlType, function(err) {
        if (err) return callback(err);
        callback(null);
      });
    }

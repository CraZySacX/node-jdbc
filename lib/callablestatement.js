...
CallableStatement.prototype.getMoreResults = function(callback) {
	this._cs.getMoreResults(function(err, hasResults) {
		if (err) {
			return callback(err);
		} else {
			return callback(null, hasResults);
		}
	});
};

CallableStatement.prototype.getResultSet = function(callback) {
	this._cs.getResultSet(function(err, resultset) {
		if (err) {
			return callback(err);
		} else {
